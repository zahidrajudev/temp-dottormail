...
function DottormailUserDashboard() {
  ...
  const getInitialdataLoad = async () => {
    ...
    await Api.post(url, { days: period })
      .then((res) => {
        ...
        if (res_data) {
          setStats([
            { icon: "news", name: "Total Lists", value: res_data?.total_lists },
            { icon: "mail", name: "Total Emails", value: res_data?.total_emails },
            { icon: "attach_money", name: "Total Spent", value: "€ " + res_data?.total_spent },
            { icon: "award_star", name: "Your Credits", value: res_data?.total_credits },
          ]);
          setPieData([
  ...
  const chartList = [
    { name: "Press Release", value: "press_releases" },
    { name: "Transactions", value: "transactions" },
  ];
  const [activeNow, setActiveNow] = useState("transactions");
  ...
  const prepareTableData = (data: any[]) => {
    ...
    const tableData = data.map((dat) => ({
      id: dat.id,
      invoice: dat?.invoice,
      gateway: <ImageBox src={dat?.gateway?.media?.path} className="h-10 max-w-20" zoom_on_hover={false} />,
      ...
      status:
        ...
        (dat.status == 2 && <div className="text-red-500 bg-red-200 w-max px-4 py-1.5 rounded-full">Canceled</div>) ||
      ...
      action: (
        <div className="text-xs flex justify-end" title="View">
          <Link
            href="/dashboard/v2/payment/history/"
            className="rounded bg-blue-950 text-white hover:bg-blue-800 px-2 py-1.5 flex items-center gap-1 text-[14px] cursor-pointer group"
          >
            View
  ...
  return (
    <Section className="space-y-8 p-4 md:p-6 lg:p-8 bg-slate-50/50 min-h-screen rounded-[32px]" permission={hasPermission("dashboard.user")} loading={loading}>
      {/* Top Control Bar */}
...
export default DottormailUserDashboard;
