...
import { checkErrors, formatNumber } from "@/lib/helper";
...
function CurrencyListPage() {
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      ...
      code: dat?.code,
      symbol: dat?.symbol,
      rate: dat?.default == 1 ? `${formatNumber(dat?.rate)} ${dat?.code}` : `1 ${appDefaultCurrency?.code} = ${formatNumber(dat?.rate)} ${dat?.code}`,
      default:
        dat.default == 1 ? (
      ...
      action: hasPermission("currency.edit") && (
        <div className="text-xs" onClick={() => actionManager("open_update", dat.id)} title="Edit">
          <Button url={`/dashboard/currency/edit?id=${dat.id}`} showIcon iconName="edit_note" px="px-3" py="py-1.5">
            Edit
          </Button>
        </div>
      ),
  ...
  return (
    <Section
      permission={hasPermission("currency.view")}
      loading={loading}
    ...
    >
      <Table
        ...
        excludeKeys={["id"]}
        showDelete={hasPermission("currency.delete")}
