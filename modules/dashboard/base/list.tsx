...
function BaseListPage() {
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      ...
      name: dat?.name,
      email: dat?.email,
      "email status": dat.email_verified_at ? (
        <div className="pl-2 pr-3 py-1 rounded-full bg-green-500/5 border border-green-700 w-max text-green-700 font-semibold">
          ...
          <div className="inline border-l border-green-700 ms-2 pl-2 text-green-700 text-xs">Verified</div>
        </div>
      ) : (
        <div className="pl-2 pr-3 py-1 rounded-full bg-red-500/5 border border-red-500 w-max text-red-600 font-semibold">
          ...
          <div className="inline border-l border-red-500 ms-2 pl-2 text-xs">Unverified</div>
        </div>
      ),
      role: dat?.role?.name,
      "account status":
