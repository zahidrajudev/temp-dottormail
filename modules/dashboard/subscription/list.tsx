...
function SubscriptionListPage() {
  ...
  const getInitialData = async () => {
    ...
    let url = "v1/dashboard/subscription";
    await api
      .post(url, data)
      .then((res) => {
        setLoading(false);
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      ...
      status:
        dat.status == 1 ? (
          ...
          <div className="pl-2 pr-3 py-1 rounded-full bg-red-500/5 border border-red-500 w-max text-red-600 font-semibold">
            ...
            <div className="inline border-l border-red-500 ms-2 pl-2 text-xs">Canceled</div>
          </div>
        ),
      recurring: dat?.recurring,
      "is default":
        dat.is_default == 1 ? (
          <div className="pl-2 pr-3 py-1 rounded-full bg-green-500/5 border border-green-700 w-max text-green-700 font-semibold">
            <SvgIcon name="check" className="size-4" filled />
      ...
      "Last Payment At _date": dat?.last_payment_at,
      "Subscription Started At _date": dat?.created_at,
      user: (
        <div>
          {" "}
          <p>{dat?.user?.name}</p> <p className="text-xs opacity-70">{dat?.user?.email}</p>{" "}
        </div>
      ),
      action: hasPermission("subscription.edit") && (
        <div className="text-xs flex items-center gap-4" onClick={() => actionManager("open_update", dat.id)} title="Edit">
          {dat?.status == 1 && (
            <Button
              onClick={() => handleCancel(dat?.id)}
              className=" flex gap-2 bg-fuchsia-500 hover:bg-fuchsia-700 text-white w-max items-center font-semibold"
              border="rounded"
              showIcon
              iconName="close"
              iconClass="size-4"
          ...
          {dat.status == 1 && dat?.is_default == 0 ? (
            <Button
              onClick={() => handleMakeDefault(dat?.id)}
              className=" flex gap-2 bg-violet-500 hover:bg-violet-700 text-white w-max items-center font-semibold"
              border="rounded"
              showIcon
              iconName="check"
              iconClass="size-4"
    ...
    >
      ...
      <div className="bg-white rounded-lg">
        <div className="flex gap-4 pl-8 pt-4 -mb-4">
          <Button url="/dashboard/subscription/add" showIcon iconName="add" border="rounded" px="px-3" py="py-1.5">
            Add Subscription
          </Button>
          <Button onClick={syncSubscription} showIcon iconName="sync" border="rounded" px="px-3" py="py-1.5">
