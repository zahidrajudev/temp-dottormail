...
import ToolListTranslation from "@/modules/dashboard/tool/translation/list";
...
function ToolListPage() {
  ...
  const { t } = pageTranslation("tool", appSelectedLocale?.code ?? "en");
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      ...
      [t("action")]: hasPermission("tool.edit") && (
  ...
  return (
    <Section
      permission={hasPermission("tool.view")}
      loading={loading}
      ...
      className="space-y-6"
    >
      <SubHeader title={t("manage_tools")} showTranslationIcon TranslationIconAction={() => setShowTranslation(true)} />
      ...
      <ToolListTranslation show={showTranslation} setShow={setShowTranslation} />
    </Section>
  );
}
