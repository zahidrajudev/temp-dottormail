...
function ToolAccountEditPage() {
  ...
  const [toolID, setToolID] = useState<any>(null);
  ...
  const handleUpdate = () => {
    let url = "v1/dashboard/master-account/update";
    let data = {
      id: actionID,
      tool_id: actionID,
      username,
    ...
    api
      .post(url, data)
      .then((res) => {
        router.push("/dashboard/tool/account?id=" + toolID);
        setLoading(false);
  ...
  const getInitialData = async (id: any) => {
    setLoading(true);
    let url = "v1/dashboard/master-account/edit";
    let data = { id };
    await api
      .post(url, data)
      .then((res) => {
        setLoading(false);
        setMainData(res.data?.tool);
        const resData = res.data?.data;
        if (resData) {
          setUsername(resData?.username ?? "");
          setPassword(resData?.password ?? "");
          const _authType = resData?.auth_type;
          setAuthType(_authType);
          if (_authType == "cookie") {
            let _cookie = resData?.cookies ?? "";
            if (_cookie) {
              if (Array.isArray(_cookie)) {
                setCookie(JSON.stringify(_cookie));
              } else {
                setCookie(_cookie);
              }
            }
          ...
          if (_authType == "token") {
            let _token = resData?.tokens ?? "";
            if (_token) {
              if (Array.isArray(_token)) {
                setToken(JSON.stringify(_token));
              } else {
                setToken(_token);
              }
            }
          ...
          const _isProxy = resData?.is_proxy ?? 0;
          if (_isProxy == 1) {
            setIsProxy(1);
            setProxyProtocol(resData?.proxy_protocol);
            setProxyLogin(resData?.is_proxy_login ?? 0);
            setProxyUserName(resData?.proxy_username ?? "");
            setProxyPassword(resData?.proxy_password ?? "");
            setProxyIp(resData?.proxy_ip ?? "");
            setProxyPort(resData?.proxy_port ?? "");
            setProxyType(resData?.proxy_type ?? "");
          }

          setMaxUseLimit(resData?.max_usage_limit ?? "");
          setToolID(resData?.tool?.id ?? "");
        }
      })
      .catch((err) => {
        setLoading(false);
  ...
  return (
    <Section permission={hasPermission("tool-account.edit")} loading={loading} className="bg-white p-8 border border-gray-200 rounded-lg space-y-10">
...
export default ToolAccountEditPage;
