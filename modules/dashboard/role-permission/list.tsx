...
function RolePermissionListPage() {
  ...
  const getInitialData = async () => {
    ...
    let url = "v1/dashboard/role-permission";
    await api
      .post(url, data)
      .then((res) => {
        setLoading(false);
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      id: dat.id,
      name: dat?.name,
      "total users": dat?.users_count,
      "Last Activity _date": dat.updated_at,
      action: hasPermission("role.edit") && (
        <div className="text-xs" onClick={() => actionManager("open_update", dat.id)} title="Edit">
          <Button url={`/dashboard/role/edit?id=${dat.id}`} showIcon iconName="edit_note" px="px-3" py="py-1.5">
            Edit
          </Button>
        </div>
      ),
  ...
  return (
    ...
    >
      <Table
        ...
        excludeKeys={["id"]}
        showDelete={hasPermission("role.delete")}
