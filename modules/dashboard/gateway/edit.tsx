...
import GatewayEditTranslation from "./translations/edit";
...
function GatewayEditPage() {
  ...
  const { appCurrencies } = useCurrencyStore();
  ...
  const handleUpdate = () => {
    let url = "v1/dashboard/gateway/update";
  ...
  const getInitialData = async (id: any) => {
    setLoading(true);
    let url = "v1/dashboard/gateway/edit";
    let data = { id };
    await api
      .post(url, data)
      .then((res) => {
        ...
        if (dat) {
          setName(dat?.translations?.name ?? "");
          setDes(dat?.translations?.des ?? "");
          if (dat?.media && typeof dat.media == "object") {
            setMedia([dat?.media]);
          } else {
            setMedia([]);
          }
          setStatus(dat?.status ?? "");
          handleMethodSelect(dat?.code ?? "", false);
          //setCode(dat?.code ?? "");
          setMode(dat?.mode ?? "");
          setKey(dat?.key ?? "");
          setPubKey(dat?.pub_key ?? "");
          setSecret(dat?.secret ?? "");
          setNumber(dat?.number ?? "");
          setCurrency(dat?.currency);
        }
      })
      .catch((err) => {
        setLoading(false);
  ...
  return (
    <>
      ...
      <Section permission={hasPermission("gateway.edit")} loading={loading} className="bg-white p-8 border border-gray-200 rounded-lg space-y-10">
        ...
        <GatewayEditTranslation show={showTranslation} setShow={setShowTranslation} />
      </Section>
    </>
  );
}
