...
import PackageListTranslation from "./translations/list";
...
function PackageListPage() {
  ...
  const handleTrash = async () => {
    try {
      let url = "v1/dashboard/package/permanent-delete";
  ...
  const prepareTableData = (data: MainData[]) => {
    const tableData = data.map((dat) => ({
      id: dat.id,
      [t("name")]: dat?.name,
      [t("price")]: formatPrice(dat?.price),
      [t("credit")]: dat?.credit,
      [`${t("last_activity")} _date`]: dat.updated_at,
      action: hasPermission("package.edit") && (
        <div className="text-xs" onClick={() => actionManager("open_update", dat.id)} title="Edit">
          <Button url={`/dashboard/package/edit?id=${dat.id}`} showIcon iconName="edit_note" px="px-3" py="py-1.5">
            {t("edit")}
          </Button>
        </div>
      ),
  ...
  return (
    <Section
      permission={hasPermission("package.view")}
      loading={loading}
    ...
    >
      ...
      <Table
        ...
        excludeKeys={["id"]}
        showDelete={hasPermission("package.delete")}
      ...
      <PackageListTranslation show={showTranslation} setShow={setShowTranslation} />
    </Section>
  );
}
